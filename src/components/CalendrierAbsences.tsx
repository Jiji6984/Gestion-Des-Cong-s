...
import { format, startOfMonth, endOfMonth, startOfWeek, endOfWeek,
         ...
         parseISO, isBefore, isAfter } from "date-fns";
...
// Map: "YYYY-MM-DD" → liste d'absences ce jour-là
...
function buildJourMap(absences: Absence[], debut: Date, fin: Date): JourMap {
  const map: JourMap = {};
  for (const a of absences) {
    const start = parseISO(a.date_debut);
    const end   = parseISO(a.date_fin);
    let cur = new Date(Math.max(debut.getTime(), start.getTime()));
    const stop = new Date(Math.min(fin.getTime(), end.getTime()));
    while (cur <= stop) {
...
export function CalendrierAbsences() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setRole(emp?.role ?? "employe");
    };
    init();
  }, []);

  /* ——— Chargement des absences du mois ——— */
  const charger = useCallback(async () => {
    ...
    let query = supabase
      ...
      `)
      .eq("statut", "approuve")
      .lte("date_debut", format(finMois, "yyyy-MM-dd"))
      .gte("date_fin",   format(debutMois, "yyyy-MM-dd"));

    // Manager → seulement son équipe
    if (role === "manager") {
      const { data: equipe } = await supabase
        .from("employes").select("id").eq("manager_id", userId);
      const ids = (equipe ?? []).map((e: any) => e.id);
      if (ids.length === 0) { setAbsences([]); setCharge(false); return; }
      query = query.in("employe_id", ids);
    }
        ...
        {chargement ? (
          ...
          <div>
            {semaines.map((semaine, si) => (
              <div
                key={si}
                className={`grid grid-cols-7 ${si < semaines.length - 1 ? "border-b border-gray-100" : ""}`}
              >
                {semaine.map((jour, ji) => {
                  const key      = dateStr(jour);
                  const entrees  = jourMap[key] ?? [];
                  const estMois  = isSameMonth(jour, moisCourant);
                  const estAujourd = isToday(jour);
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[100px] p-2 ${ji < 6 ? "border-r border-gray-100" : ""} ${
                        !estMois   ? "bg-gray-50/50" :
                        estWeekend ? "bg-gray-50/30" : "bg-white"
                      }`}
                    >
                      {/* Numéro du jour */}
